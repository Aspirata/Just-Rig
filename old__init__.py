...
class JustRigUI(bpy.types.Panel):
    ...
    def show_error(self, error: str):
        ...
        row = box.row()
        ds_link_op = row.operator("wm.url_open", text="Aspirata's Discord Server", icon='URL')
    ...
    def draw_facial_settings_section(self, layout):
        ...
        if self.ui_props.facial_settings_expanded:
            ...
            if self.ui_props.eyes_settings_expanded:
                ...
                tbox = sbox.box()
                row = tbox.row()
                row.label(text="Eyes Colors:", icon='IMAGE')
                ...
                tbox = sbox.box()
                row = tbox.row()
                row.label(text="Irises Colors:", icon='IMAGE')
                
                col = tbox.column(align=True)

                row1 = col.row(align=True)
                row1.prop(self.eye_color_inputs["R Iris"], 'default_value', text="")
                row1.prop(self.eye_color_inputs["L Iris"], 'default_value', text="")
                row1.enabled = self.settings_bones["Facial Settings"]["Eyes"]
